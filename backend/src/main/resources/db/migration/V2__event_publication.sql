...
-- DDL taken from spring-modulith-events-jdbc (schemas/v2/schema-postgresql.sql) so it matches what
-- the framework expects; it is applied here rather than by schema initialization at startup.
-- The archive table receives completed publications when the completion mode is set to archive,
-- which keeps the hot table small once event volume grows.
...
CREATE INDEX ix_event_publication_archive_serialized_event_hash
    ON event_publication_archive USING HASH (serialized_event);
CREATE INDEX ix_event_publication_archive_completion_date
